    ...
    <div className="space-y-1.5">
      ...
      {(scan.changed_files_count > 0 || scan.untracked_files_count > 0 || scan.staged_files_count > 0) && (
        <div className="flex gap-3 text-xs text-slate-500">
          ...
          {scan.changed_files_count > 0 && (
            <span className="text-yellow-500">{scan.changed_files_count} modified</span>
          )}
          {scan.untracked_files_count > 0 && (
            <span>{scan.untracked_files_count} untracked</span>
          )}
        </div>
      )}
      ...
      {scan.last_commit_hash && (
        <div className="flex items-start gap-1.5 text-xs text-slate-500">
          <GitCommit size={11} className="mt-0.5 shrink-0 text-slate-600" />
          <div>
            <span className="font-mono text-slate-600 mr-1.5">{shortHash(scan.last_commit_hash)}</span>
            <span className="text-slate-400">{scan.last_commit_message}</span>
            {scan.last_commit_date && (
              <span className="ml-1.5 text-slate-600">{relativeTime(scan.last_commit_date)}</span>
            )}
          </div>
        </div>
      )}
